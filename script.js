$(document).ready(function () {
  ...
  function displayCurrentDay() {
    // Get the current date and format it (e.g., "Wednesday, August 3, 2023")
    const currentDay = dayjs().format("dddd, MMMM D, YYYY");
    // Set the formatted date to the element with id "currentDay"
  ...
  function generateTimeBlocks() {
    ...
    // Loop through each hour from the startHour to endHour (inclusive)
    for (let i = startHour; i <= endHour; i++) {
      // Format the hour for display (e.g., "9AM", "10AM", ...)
  ...
  function updateColorCoding() {
    ...
    $(".time-block").each(function () {
      // Extract the hour from the time block's ID (e.g., "hour-9" -> 9)
  ...
  updateColorCoding();
  loadSavedEvents();
  // Update color coding every 60 seconds (60000 milliseconds)
